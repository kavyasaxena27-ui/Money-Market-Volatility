...
# -----------------------------------------------------------------------------
# 25bps normalisation
# Follows paper Section 3: IRFs scaled to 25bps peak response in 3m OIS rate
# Step 1: run LP for 3m OIS outcome at mean vol
# Step 2: find peak response
# Step 3: scale_factor = 0.25 / peak_response
# Step 4: pass to all run_analysis() calls via scale argument
#
# Update PARAM_OIS_OUTCOME to match your 3m OIS/SONIA rate column name
# Set PARAM_NORMALISE = FALSE to skip normalisation
# -----------------------------------------------------------------------------

PARAM_OIS_OUTCOME <- "gbp3mois"
PARAM_NORMALISE   <- TRUE

compute_scale_factor <- function(data, shock, interaction_var,
                                  ois_outcome, horizons, iv,
                                  cumulative = FALSE) {
  cat(glue("\nComputing 25bps normalisation from {ois_outcome}...\n"))

  ois_res <- run_analysis(
    data            = data,
    outcome         = ois_outcome,
    shock           = shock,
    interaction_var = interaction_var,
  ...
  peak <- ois_res$fig_data %>%
    filter(curve == "Mean state") %>%
    summarise(peak = max(abs(est), na.rm = TRUE)) %>%
    pull(peak)

  scale_factor <- 0.25 / peak

  cat(glue("  Peak OIS response: {round(peak, 4)}\n"))
  cat(glue("  Scale factor:      {round(scale_factor, 4)}\n\n"))

  return(scale_factor)
}

# Compute scale factor
scale_factor <- if (PARAM_NORMALISE) {
  compute_scale_factor(
    data            = df,
    shock           = "target",
    interaction_var = "centered_vol",
    ois_outcome     = PARAM_OIS_OUTCOME,
    horizons        = PARAM_HORIZONS,
    iv              = FALSE
  )
} else {
...
# Re-run spec grid with normalisation applied
use_grid_normalised <- spec_grid %>%
  mutate(
    results = pmap(
      list(outcome, shock, interaction, cumulative, iv),
      function(out, shk, int, cum, iv) {
      ...
      .progress = "Running normalised LPs..."
    )
  )

use_grid_normalised$results %>%
  map("plot") %>%
  wrap_plots(ncol = 3)
